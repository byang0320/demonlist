...
import { listRankedLevels } from '@/features/levels/queries'
...
export default async function DemonListPage() {
  const levels = await listRankedLevels()
      ...
      <div className="mx-auto w-full max-w-280">
        <header className="flex flex-col items-start justify-between gap-5 pb-6 pt-4 sm:flex-row sm:items-end sm:gap-8 sm:pb-9">
          ...
          <p className="mb-1 flex-1 text-sm text-[#8c97b2] sm:flex-none sm:text-right">
            <span className="mr-1 text-2xl font-bold tracking-[-0.05em] text-white sm:block">
              {levels.length}
            </span>{' '}
            {levels.length === 1 ? 'placed level' : 'placed levels'}
          </p>
        </header>
